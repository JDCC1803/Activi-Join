@extends('dashboard')
@section('mayor')
<div class="container px-4 md:px-0 max-w-6xl mx-auto -mt-40">
    @if(count($ver)>0)
    <div class="max-w-4xl flex items-center h-auto lg:h-screen flex-wrap mx-auto mt-32 lg:mt-20">
        <!--Main Col-->
        <div id="profile" class="w-full lg:w-3/5 rounded-lg lg:rounded-l-lg lg:rounded-r-none shadow-2xl bg-white opacity-75 mx-6 lg:mx-0">
            <div class="p-4 md:p-12 text-center lg:text-left">
                <!-- Image for mobile view-->
                <div class="block lg:hidden rounded-full shadow-xl mx-auto -mt-16 h-48 w-48 bg-cover bg-center">@livewire('foto-perfila')</div>
                @foreach ($ver as $object)
                <h1 class="text-3xl font-bold pt-8 lg:pt-0">{{$object->name}}</h1>
                ...
                <p class="pt-2 text-gray-600 text-xs lg:text-sm flex items-center justify-center lg:justify-start"><svg class="h-4 fill-current text-green-700 pr-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                    ...
                    </svg>{{$object->grado}}{{$object->grupo}}</p>
                <p class="pt-8 text-sm font-bold">Breve descripción de tu persona:</p>
                <p class="pt-2 text-sm">{{$object->descripcion}}</p>
                @endforeach
        ...
        <div class="w-full lg:w-2/5">
            @livewire('foto-perfila')
        </div>
    </div>
    @livewire('ver-perfila')
    @else
